...
- **The Docket (daily, added 30 Jul 2026):** alongside the featured carousel, every daily run also produces a single roundup slide listing ALL significant decisions from the previous business day (or latest verified window): case name (v in accent blue), one-line holding, citation line, thin rules between entries, max 5 entries per slide (overflow → second slide). Title "The Docket." + date top-right, subline states the window honestly. Template: `tools/carousel/docket.py`. Post as slide 2 of the daily post or as a story.
