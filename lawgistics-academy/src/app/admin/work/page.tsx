    ...
    <div className="space-y-6">
      <section>
        ...
        <p className="mt-3 max-w-2xl text-slate">
          A piece of work, or something to read. Interns put their name on a task, hand it
          in here, and you mark it: good, or needs another go, and why. Nothing with a
          client&apos;s name in it, on either side.
        </p>
        {waiting > 0 ? (
      ...
      <div className="space-y-3">
        {posts.map(({ post, claims, submissions, waiting: toMark }) => (
          <Card key={post.id}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
                  ...
                  {post.kind === 'task' ? (
                    <Pill>{post.scope === 'one' ? 'One person' : 'Everyone'}</Pill>
                  ) : null}
                  <Pill>
                ...
                {post.kind === 'task' ? (
                  <p className="mt-1 text-sm text-muted">
                    {claims} name{claims === 1 ? '' : 's'} on it, {submissions} handed in
                  </p>
                ) : null}
              </div>
              ...
              >
                {post.kind === 'task' && submissions > 0 ? 'Open and mark' : 'Edit'}
              </Link>
            </div>
          </Card>
        ))}
