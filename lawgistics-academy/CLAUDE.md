...
- **Two staff roles, and the line between them holds.** An administrator writes
  ...
  Exactly seven actions accept a coach, named in `tests/authorisation-contract`,
  and a test fails if an eighth quietly does. Two of the seven are the coach's
...
- Migrations run to `0015`. `supabase/UPDATE.sql` is the one-paste update for a
...
- 219 tests, 95 schema guarantees against a real Postgres, 96 page and device
